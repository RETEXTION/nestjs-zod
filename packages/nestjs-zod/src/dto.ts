...
import { toJSONSchema, $ZodType, JSONSchema, $ZodRegistry } from "zod/v4/core";
...
>(schema: TSchema, registry?: $ZodRegistry<{description: string}>, params?: SchemaParams) {
  class AugmentedZodDto {
    ...
    static get Output() {
      ...
      class AugmentedZodDto {
        ...
        public static _OPENAPI_METADATA_FACTORY() {
          return openApiMetadataFactory({ schema: this.schema, params: params, io: "output" });
        }
      }
    ...
    public static _OPENAPI_METADATA_FACTORY() {
      return openApiMetadataFactory({ schema: this.schema, params: params, io: "input" });
    }
  }
...
function openApiMetadataFactory({ 
  ...
  const { $defs, $schema, ...generatedJsonSchema } = generateJsonSchema(schema, io, params);
...
function generateJsonSchema(
  ...
  const generatedJsonSchema = '_zod' in schema ? toJSONSchema(schema, {
    io,
    target: params?.target ?? 'openapi-3.0',
    metadata: params?.metadata ?? z.globalRegistry,
    override: ({ jsonSchema }) => {
